# [pandas](https://pandas.pydata.org/)

- [pandas](#pandas)
  - [Q&A](#qa)

## Q&A

- [Pandas How to replace values based on Conditions](https://kanoki.org/2019/07/17/pandas-how-to-replace-values-based-on-conditions/)

- [Select Rows & Columns by Name or Index in DataFrame using loc & iloc](https://thispointer.com/select-rows-columns-by-name-or-index-in-dataframe-using-loc-iloc-python-pandas/)

- [Get first element of Series without knowing the index](https://stackoverflow.com/questions/24273130/get-first-element-of-series-without-knowing-the-index)
