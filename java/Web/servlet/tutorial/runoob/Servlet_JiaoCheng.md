...
- [Servlet 教程](#servlet-教程)
  ...
  - [Servlet 数据库访问](#servlet-数据库访问)
  - [Servlet 文件上传](#servlet-文件上传)
  - [Servlet 处理日期](#servlet-处理日期)
  - [Servlet 网页重定向](#servlet-网页重定向)
  - [Servlet 点击计数器](#servlet-点击计数器)
  - [Servlet 自动刷新页面](#servlet-自动刷新页面)
  - [Servlet 发送电子邮件](#servlet-发送电子邮件)
  - [Servlet 包](#servlet-包-1)
    - [创建包中的 Servlet](#创建包中的-servlet)
    - [编译包中的 Servlet](#编译包中的-servlet)
    - [Servlet 打包部署](#servlet-打包部署)
  - [Servlet 调试](#servlet-调试)
  - [Servlet 国际化](#servlet-国际化)
...
除了上述的三种方式，Servlet 还提供了 HttpSession 接口，该接口提供了一种**跨多个页面请求**或访问网站时识别用户以及存储有关用户信息的方式。
...
WEB-INF 子目录中包含应用程序的**部署描述符**，名为 `web.xml`。所有的 HTML 文件都位于顶级目录 myapp 下。对于 admin 用户，您会发现 ROOT 目录是 myApp 的父目录。
...
到这里，您基本上已经完成了，现在让我们使用 `<Tomcat-installation-directory>\bin\startup.bat`（在 Windows 上）或 `<Tomcat-installation-directory>/bin/startup.sh`（在 Linux/Solaris 等上）启动 tomcat 服务器，最后在浏览器的地址栏中输入 `http://localhost:8080/MyServlet`。
